...
birthweight_df[categorical_vars] = lapply(birthweight_df[categorical_vars], factor)
...
birthweight_model = lm(bwt ~ babysex + bhead + mheight + ppbmi + smoken, data = birthweight_df)
...
```{r}
cv_error = function(df, model_formula) {
  crossv_mc(df, 100) |>
    mutate(
      test_results = map(.x = test, .f = ~lm(model_formula, data = .x)),
      test_predictions = map2(.x = test_results, .y = test, .f = ~add_predictions(.y, .x)),
      test_residuals = map2(.x = test_predictions, .y = test, .f = ~add_residuals(.y, .x))
    ) |>
    unnest(test_residuals) |>
    summarize(mse = mean(resid^2))
}

model1_formula = bwt ~ blength + gaweeks
model1_error = cv_error(birthweight_df, model1_formula)

model2_formula = bwt ~ bhead * blength * babysex
model2_error = cv_error(birthweight_df, model2_formula)
